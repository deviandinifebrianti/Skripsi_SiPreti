...
class MTCNNDetect(object):
    path = os.path.join(settings.MEDIA_ROOT, 'models', '20170512-110547.pb')
    def __init__(self, face_rec_graph, model_path = path, threshold = [0.6, 0.7, 0.7], factor = 0.709, scale_factor = 1):
        '''
...
class Network(object):
    ...
    def fc(self, inp, num_out, name, relu=True):
        with tf.compat.v1.variable_scope(name):
            ...
            fc = tf.matmul(feed_in, weights) + biases
        if relu:
            fc = tf.nn.relu(fc)
